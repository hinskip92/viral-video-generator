...
def create_vertical_clips(input_file: str, viral_clips: List[Dict[str, Any]], output_folder: str) -> None:
    ...
    try:
        # Instantiate outside the loop
        video = VideoFileClip(input_file)
        for i, clip_info in enumerate(viral_clips, 1):
            start, end = clip_info['timecodes']
            # Try calling subclipped here (Changed from subclip)
            segment = video.subclipped(start, end)

            # Resize and crop for vertical format using built-in methods (MoviePy v2+)
            vertical_segment = segment.resized(height=1920)

            # Safeguard: Only crop if width is available after resize
            if hasattr(vertical_segment, "w"):
                vertical_segment = vertical_segment.cropped(x_center=vertical_segment.w / 2, width=1080)
            else:
                logging.warning(
                    f"Clip segment {i} doesn't have width attribute after resize. Skipping crop."
                )
        ...
        # Specific handling for the subclip/subclipped error
        if 'subclip' in str(ae).lower(): # Check for both names just in case
            logging.error(f"AttributeError: 'subclip' or 'subclipped' not found on VideoFileClip object for {input_file}. This might indicate an issue with the moviepy installation, version mismatch, or the video file itself.")
        else:
            logging.error(f"AttributeError creating vertical clips for {input_file}: {str(ae)}")
    except Exception as e:
        logging.error(f"Error creating vertical clips for {input_file}: {str(e)}")
    finally:
        # Explicitly close the video file if it was opened
        if video:
            try:
                video.close() # Important for releasing file handles
                logging.debug(f"Closed video file: {input_file}")
            except Exception as close_err:
                logging.error(f"Error closing video file {input_file}: {close_err}")
